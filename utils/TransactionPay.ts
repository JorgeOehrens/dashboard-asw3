...
const getAllTransactionsPay = async (): Promise<Transaction[]> => {
    try {
        if (window.ethereum) {
            ...
            for (const token of MarketData) {
                const transactionContract = createEthereumContract(token.adress_token);
...
export default getAllTransactionsPay;
